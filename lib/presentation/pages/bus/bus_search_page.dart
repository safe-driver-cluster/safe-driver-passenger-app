    ...
    try {
      ...
      for (var doc in querySnapshot.docs) {
        ...
        buses.add({
          ...
          'nextArrival': '${(5 + (buses.length * 3))} min', // Simulated arrival time
  ...
  void _searchByRoute() async {
    ...
    try {
      ...
      for (var doc in querySnapshot.docs) {
        ...
        bool matchesFrom = fromQuery.isEmpty || route.contains(fromQuery) || address.contains(fromQuery);
        bool matchesTo = toQuery.isEmpty || route.contains(toQuery) || address.contains(toQuery);
  ...
  void _searchByLocation() async {
    ...
    try {
      ...
      for (var doc in querySnapshot.docs) {
        ...
        if (route.contains(searchQuery) || address.contains(searchQuery) || busNumber.contains(searchQuery)) {
          buses.add({
            'id': doc.id,
            'busNumber': data['busNumberPlate'] ?? 'N/A',
            'routeName': data['route'] ?? 'Unknown Route',
